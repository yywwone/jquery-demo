...
$(function(){
	...
	function autoSlide(){
		...
		if(imgNow==imgDiv.size()-1)
			{
				imgNow=0;
				imgDiv.eq(0).css({"opacity":1});
				imgDiv.eq(imgDiv.size()-1).css({"opacity":0.2})
			}
		else
			{
				imgNow++;
		...
		if(imgNow==0)		//显示第一张图片
			{
				...
				imgDiv.eq(imgNow).css({"opacity":1}).siblings().css({"opacity":0.2});
		}
	}
	
	//鼠标影响图片轮换的方法主体
	function mouseSlide(){
		//判断鼠标所在图片是否已经轮换过，选择右边图片移动或者左边图片移动
		if(imgMouse>imgNow)
				...
				for(var i=imgNow+1;i<=imgMouse;i++)
					{
						...
						imgDiv.eq(i).css({"opacity":1}).siblings().css({"opacity":0.2});
					}
				//重置当前图片的索引
				imgNow=imgMouse;
				...
				for(var i=imgNow;i>imgMouse;i--)
					{
						...
						imgDiv.eq(imgMouse).siblings().children().show();
						//图片运动
						imgDiv.eq(i).stop().animate({left:imgOuterWidth-(imgDiv.size()-i)*edgeDistance+'px',},1000);
						imgDiv.eq(i-1).css({"opacity":1}).siblings().css({"opacity":0.2});
					}
				//重置当前图片的索引
				imgNow=imgMouse;
			}
